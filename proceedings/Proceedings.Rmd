...
Convert2Cite <- function( names ) {
  # Get last names (list)
  fam <- as.list( sub(pattern=".* ", replacement="", x=names) )
  # Pattern to search for initials
  regex <- '(?<=^|\\s)[[:alpha:]]' 
  # Get initials
  initials <- regmatches( x=names, 
    m=gregexpr(pattern=regex, text=names, perl=TRUE) )
  # Omit last initial
  inits <- lapply( X=initials, FUN=function(x) head(x, -1) )
  # Combine initials
  init <- lapply( X=inits, FUN=function(x) paste(x, collapse=".") )
  # Error if these aren't the same length
  if( length(fam) != length(init) )
    stop( "Different length for family names and initials" )
  # Combine last name and initials
  famInit <- paste( fam, init, sep=", ")
  # Add a period
  res <- paste( famInit, ".", sep="" )
  # Return the results
  return( res )
...
<!-- Authors -->
\newcommand{\trAuthsLong}{`r PasteNicely(Authors)`}
