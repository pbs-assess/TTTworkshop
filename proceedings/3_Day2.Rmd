...
allows analysts to intersperse text and R code in a single file. The text and
results of the R code are then
...
a markup language that is compatible with LaTeX, but easier to learn.
Basic formatting options were presented. Some recommendations included naming
code chunks using unique and useful identifiers (to aid in using caches and
...
and oceanographic survey data; this is called the OBIS-ENV-DATA schema. He listed
several online repositories for biological data that use DWC data
...
maintain stocks above Limit Reference Points (Provision 6.1(2)), and rebuild
stocks above Limit Reference Points within specified time periods (Provision
...
uncertainty). In general, defensible science advice means that the advice meets
current standards of acceptable scientific and fisheries management practice,
including reproducibility, a key element of this workshop. The suite of
