...
\item During this discussion, I re-ran the model with the survey included, and then presented the results, automatically produced as a single .pdf file.
...
\frame{\frametitle{Example \LaTeX\ code}
\verbatim{
  \\begin{table}[t] 
  \\caption{Summary of how the four Series {\\bf A}, {\\bf B}, {\\bf C} and {\\bf D}
    ...
    data for each Series are available.}
...
\\textbf{Series A} -- 1997-2017 stations north of WCVI, with catch rates based on first 20~hooks only (which is all we have for 1997-2002 and 2013).
}
}
